...
public class EnrichmentAppWithoutExplode {
...
//                        "'properties.group.id' = '" + config.getString("kafka.group_id") + "'," +
    ...
    public static void createMsIpExploded() {
        String msIp =
                "CREATE TABLE ms_ip_exploded (" +
                        "start_time TIMESTAMP," +
                        "imsi BIGINT," +
                        "msisdn BIGINT," +
                        "ms_ip_address STRING," +
                        ...
                        ") WITH (" +
                        "'connector' = 'jdbc'," +
                        "'url' = '" + config.getString("ms_ip_exploded.url") + "'," +
                        "'table-name' = '" + config.getString("ms_ip_exploded.dbtable") + "'," +
                        "'username' = '" + config.getString("ms_ip_exploded.user") + "'," +
                        "'password' = '" + config.getString("ms_ip_exploded.password") + "'," +
                        "'lookup.cache' = 'PARTIAL'," +
                        "'lookup.partial-cache.max-rows' = '" + config.getString("ms_ip_exploded.lookup.partial-cache.max-rows") + "'," +
                        "'lookup.partial-cache.expire-after-write' = '" + config.getString("ms_ip_exploded.lookup.partial-cache.expire-after-write") + "'" +
                        ")";
